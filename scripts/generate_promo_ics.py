...
def generate_weather_ics() -> bytes:
    ...
    now = datetime(2026, 3, 13, 6, 0, tzinfo=TZ)

    # Date window: Sat Mar 14 - Thu Mar 19 (6 days, easy to find from today)
    sat0 = date(2026, 3, 14)
    sun0 = date(2026, 3, 15)
    mon = date(2026, 3, 16)
    tue = date(2026, 3, 17)
    ...
    # --- Saturday: mild, partly cloudy (no drama) ---
    sat0_desc = "\n".join([
        _hourly_line(6, "\u2601\ufe0f", 8),
        _hourly_line(7, "\u2601\ufe0f", 9),
        _hourly_line(8, "\u26c5", 10),
    ...
    events_data.append({
        "uid": _uid(f"{sat0}:{LOCATION}"),
        "summary": "AM \u26c510\u00b0/13\u00b0 \u00b7 PM \u26c511\u00b0/14\u00b0",
        "start": sat0,
        "end": sat0 + timedelta(days=1),
        "description": sat0_desc,
    })
    ...
    events_data.append({
        "uid": _uid(f"{sun0}:{LOCATION}"),
        "summary": "\u26a0\ufe0f \u2602\ufe0f AM \U0001f327\ufe0f10\u00b0/12\u00b0 \u00b7 PM \U0001f327\ufe0f8\u00b0/13\u00b0",
        "start": sun0,
        "end": sun0 + timedelta(days=1),
        "description": sun0_desc,
    })

    # Sunday timed rain warning 09:00-14:00 (overlaps with Lunch with Sara 12:00-13:00)
    sun0_rain_desc = "\n".join([
        _hourly_line(9, "\U0001f327\ufe0f", 11, precip_mm=1.0, rain_pct=85),
        _hourly_line(10, "\U0001f327\ufe0f", 12, precip_mm=0.9, rain_pct=82),
        _hourly_line(11, "\U0001f327\ufe0f", 12, precip_mm=0.7, rain_pct=78),
    ...
    events_data.append({
        "uid": _uid(f"2026-03-15T09:00:00:{LOCATION}:rain"),
        "summary": "\u2602\ufe0f 4.2mm",
        "start": _dt(sun0, 9, 0),
        "end": _dt(sun0, 14, 0),
        "description": sun0_rain_desc,
    })
    ...
    events_data.append({
        "uid": _uid(f"{tue}:{LOCATION}"),
        "summary": "AM \u2600\ufe0f14\u00b0/17\u00b0 \u00b7 PM \u2600\ufe0f17\u00b0/21\u00b0",
        "start": tue,
        "end": tue + timedelta(days=1),
        "description": tue_desc,
    })

    # --- Wednesday: beautiful sunny day ---
    wed_desc = "\n".join([
        _hourly_line(6, "\u2600\ufe0f", 14),
        _hourly_line(7, "\u2600\ufe0f", 15),
        _hourly_line(8, "\u2600\ufe0f", 17),
    ...
    # Wednesday timed sunny event 10:00-17:00
    wed_sunny_desc = "\n".join([
        _hourly_line(10, "\u2600\ufe0f", 21),
        _hourly_line(11, "\u2600\ufe0f", 22),
        _hourly_line(12, "\u2600\ufe0f", 23),
    ...
    events_data.append({
        "uid": _uid(f"2026-03-18T10:00:00:{LOCATION}:sunny"),
        "summary": "\u2600\ufe0f 23 ~ 25\u00b0C",
        "start": _dt(wed, 10, 0),
        "end": _dt(wed, 17, 0),
        "description": wed_sunny_desc,
    })
...
def generate_life_ics() -> bytes:
    ...
    life_events = [
        # Saturday
        {"summary": "Farmers market", "start": _dt(sat0, 9, 30), "end": _dt(sat0, 11, 0), "location": "Viktualienmarkt"},
        {"summary": "Coffee with Alex", "start": _dt(sat0, 15, 0), "end": _dt(sat0, 16, 0), "location": ""},

        # Sunday — Lunch with Sara overlaps the rain warning (12:00-13:00 inside 09:00-14:00)
        {"summary": "Lunch with Sara", "start": _dt(sun0, 12, 0), "end": _dt(sun0, 13, 0), "location": "Marienplatz"},

        # Monday
        {"summary": "Team standup", "start": _dt(mon, 9, 0), "end": _dt(mon, 9, 30), "location": ""},
        {"summary": "1:1 with Maria", "start": _dt(mon, 14, 0), "end": _dt(mon, 14, 30), "location": ""},

        # Tuesday
        {"summary": "Team standup", "start": _dt(tue, 9, 0), "end": _dt(tue, 9, 30), "location": ""},
        {"summary": "Dentist", "start": _dt(tue, 14, 0), "end": _dt(tue, 15, 0), "location": ""},
        {"summary": "Dinner with Tom", "start": _dt(tue, 19, 0), "end": _dt(tue, 20, 30), "location": "Augustiner Keller"},

        # Wednesday — empty on purpose (nice sunny day, no plans)

        # Thursday
        {"summary": "Team standup", "start": _dt(thu, 9, 0), "end": _dt(thu, 9, 30), "location": ""},
        {"summary": "Sprint planning", "start": _dt(thu, 10, 0), "end": _dt(thu, 11, 0), "location": ""},
    ]
